...
int nvt_output_data(int *buffer, struct chip_data_nt36523 *ts_data,
...
static void nvt_pen_data_read(struct seq_file *s, struct chip_data_nt36523 *chip_info, DEBUG_READ_TYPE read_type)
{
	...
	xdata_pen_tip_x = kzalloc(tx_num * pen_rx_num * sizeof(int32_t), GFP_KERNEL);
	if (!xdata_pen_tip_x) {
		TPD_INFO("%s, xdata_pen_tip_x malloc memory failed\n", __func__);
		return;
	}

	xdata_pen_tip_y = kzalloc(pen_tx_num * rx_num * sizeof(int32_t), GFP_KERNEL);
	if (!xdata_pen_tip_y) {
		TPD_INFO("%s, xdata_pen_tip_y malloc memory failed\n", __func__);
		return;
	}

	xdata_pen_ring_x = kzalloc(tx_num * pen_rx_num * sizeof(int32_t), GFP_KERNEL);
	if (!xdata_pen_ring_x) {
		TPD_INFO("%s, xdata_pen_ring_x malloc memory failed\n", __func__);
		return;
	}

	xdata_pen_ring_y = kzalloc(pen_tx_num * rx_num * sizeof(int32_t), GFP_KERNEL);
	if (!xdata_pen_ring_y) {
	...
	kfree(xdata_pen_tip_x);
	kfree(xdata_pen_tip_y);
	kfree(xdata_pen_ring_x);
	kfree(xdata_pen_ring_y);
}
					 ...
					 struct auto_testdata *nvt_testdata, struct test_item_info *p_test_item_info)
{
	...
	for (m = 0; m < item_cnt; m++) {
		...
		if (item_head->item_limit_type == LIMIT_TYPE_NO_DATA) {
			...
			case TYPE_PEN_X_TIP:
				chip_info->p_nvt_autotest_offset->pen_tip_x_data_p = (int32_t *)(
							ts->com_test_data.limit_fw->data + item_head->top_limit_offset);
				chip_info->p_nvt_autotest_offset->pen_tip_x_data_n = (int32_t *)(
							ts->com_test_data.limit_fw->data + item_head->floor_limit_offset);
			break;
			case TYPE_PEN_Y_TIP:
				chip_info->p_nvt_autotest_offset->pen_tip_y_data_p = (int32_t *)(
							ts->com_test_data.limit_fw->data + item_head->top_limit_offset);
				chip_info->p_nvt_autotest_offset->pen_tip_x_data_n = (int32_t *)(
							ts->com_test_data.limit_fw->data + item_head->floor_limit_offset);
			break;
			case TYPE_PEN_X_RING:
				chip_info->p_nvt_autotest_offset->pen_ring_x_data_p = (int32_t *)(
							ts->com_test_data.limit_fw->data + item_head->top_limit_offset);
				chip_info->p_nvt_autotest_offset->pen_ring_x_data_n = (int32_t *)(
							ts->com_test_data.limit_fw->data + item_head->floor_limit_offset);
			break;
			case TYPE_PEN_Y_RING:
				chip_info->p_nvt_autotest_offset->pen_ring_y_data_p = (int32_t *)(
							ts->com_test_data.limit_fw->data + item_head->top_limit_offset);
				chip_info->p_nvt_autotest_offset->pen_ring_y_data_n = (int32_t *)(
							ts->com_test_data.limit_fw->data + item_head->floor_limit_offset);
			break;
			case TYPE_PEN_X_TIP_NOISE:
				chip_info->p_nvt_autotest_offset->pen_tip_x_noise_data_p = (int32_t *)(
							ts->com_test_data.limit_fw->data + item_head->top_limit_offset);
				chip_info->p_nvt_autotest_offset->pen_tip_x_noise_data_n = (int32_t *)(
							ts->com_test_data.limit_fw->data + item_head->floor_limit_offset);
			break;
			case TYPE_PEN_Y_TIP_NOISE:
				chip_info->p_nvt_autotest_offset->pen_tip_y_noise_data_p = (int32_t *)(
							ts->com_test_data.limit_fw->data + item_head->top_limit_offset);
				chip_info->p_nvt_autotest_offset->pen_tip_y_noise_data_n = (int32_t *)(
							ts->com_test_data.limit_fw->data + item_head->floor_limit_offset);
			break;
			case TYPE_PEN_X_RING_NOISE:
				chip_info->p_nvt_autotest_offset->pen_ring_x_noise_data_p = (int32_t *)(
							ts->com_test_data.limit_fw->data + item_head->top_limit_offset);
				chip_info->p_nvt_autotest_offset->pen_ring_x_noise_data_p = (int32_t *)(
							ts->com_test_data.limit_fw->data + item_head->floor_limit_offset);
			break;
			case TYPE_PEN_Y_RING_NOISE:
				chip_info->p_nvt_autotest_offset->pen_ring_y_noise_data_p = (int32_t *)(
							ts->com_test_data.limit_fw->data + item_head->top_limit_offset);
				chip_info->p_nvt_autotest_offset->pen_ring_y_noise_data_n = (int32_t *)(
							ts->com_test_data.limit_fw->data + item_head->floor_limit_offset);
			break;
			default:
